...
once an event builds successfully, the build script deletes its folder here (and
the category folder too, once every event inside it is built). This folder is
also excluded from git (see `.gitignore`) and from the published site (Jekyll
ignores any `_`-prefixed directory), so nothing here is ever committed or
served — it only exists on your local machine between dropping photos in and
running the build.
